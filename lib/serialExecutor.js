"use strict";
/**
serialExecutor.js has methods to enumerate campaigns, create jobs
from campaigns, run through the jobs and trigger result analysis
*/
...
class SerialExecutor {
  constructor(processId, jobModel, campaignModel, tabSequence) {
    ...
    this._runTime = 0;
    this._accumulator = {}; // object to register run state and results, by campaign ID
  }
  // the value of 'runTime' depends on the interval string from the command line
  // this code sets runTime to *start of* the current hour/4 hour period/day/week
  // depending on what interval we're running on
  ...
  loop(callback) {
    this._campaignModel.getCampaignForRunning(this._processId, this._runTime, (error, campaign) => {
      ...
      async.series([
        ...
        //recursion is stopped when no other campaigns could be found
  ...
  _comparePreviousRunResults(campaign, callback) {
    ...
    //can't compare results between runs if only one result exists
    ...
    async.parallel({
      ...
      let allResults = allJobs.map((r) => r.jobResults);
